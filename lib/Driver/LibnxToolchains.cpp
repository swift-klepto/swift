...
std::string
toolchains::Libnx::sanitizerRuntimeLibName(StringRef Sanitizer,
                                                 bool shared) const {
...
toolchains::Libnx::constructInvocation(const InterpretJobAction &job,
...
ToolChain::InvocationInfo toolchains::Libnx::constructInvocation(
...
std::string toolchains::Libnx::getDefaultLinker() const {
...
std::string toolchains::Libnx::getTargetForLinker() const {
  return getTriple().str();
}

bool toolchains::Libnx::addRuntimeRPath(const llvm::Triple &T,
...
ToolChain::InvocationInfo
toolchains::Libnx::constructInvocation(const DynamicLinkJobAction &job,
                                             const JobContext &context) const {
  ...
  if (const Arg *A = context.Args.getLastArg(options::OPT_use_ld)) {
    ...
    llvm_unreachable("must specify path to devkitA64 bin in -use-ld");
  }

  Arguments.push_back("--driver-mode=g++");

  if (const Arg *A = context.Args.getLastArg(options::OPT_specs)) {
    Arguments.push_back(context.Args.MakeArgString(Twine("-specs=") + A->getValue()));
  }
  else {
    llvm_unreachable("must specify path to klepto specs in -specs");
  }

  Arguments.push_back("-fPIE");

  Arguments.push_back("-Xclang");
  Arguments.push_back("-target-feature");
  Arguments.push_back("-Xclang");
  Arguments.push_back("+read-tp-soft");
...
ToolChain::InvocationInfo
toolchains::Libnx::constructInvocation(const StaticLinkJobAction &job,
                               const JobContext &context) const {
   assert(context.Output.getPrimaryOutputType() == file_types::TY_Image &&
         "Invalid linker output type.");
